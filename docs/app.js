...
const yDT       = Math.round(relTop(pDTEl)    * sy);const title = String(state.text.title || "").slice(0, TITLE_MAX);
    ...
    // Tamaño (selector estilo mockup: radio + tamaño + medidas + precio)
    ...
    // Precio actual (mantenemos la lógica de cobro en $20 MXN)
    const PRICE_NOW = "$20.00";
    const PRICE_CCY = "MXN";

    // Precios "antes" solo visuales (para UI como el mockup)
    const PRICE_WAS = {
      S: "$99.00",
      M: "$149.00",
      L: "$199.00",
    ...
    function renderSizeList(){
      ...
      EXPORT_SIZES.forEach(sz => {
        ...
        btn.type = "button";
        btn.className = "sizeOption" + (state.export.sizeKey === sz.key ? " active" : "");
        btn.setAttribute("data-size", sz.key);

        const was = PRICE_WAS[sz.key] || "";

        btn.innerHTML = `
          <span class="sizeOption__radio" aria-hidden="true"></span>
          <div class="sizeOption__key">${sz.key}</div>
          <div class="sizeOption__meta">${sz.sub}</div>
          <div class="sizeOption__price">
            <div class="sizeOption__was">${was} ${PRICE_CCY}</div>
            <div class="sizeOption__now">
              <span class="sizeOption__nowValue">${PRICE_NOW}</span>
              <span class="sizeOption__ccy">${PRICE_CCY}</span>
            </div>
          </div>
        `;
    ...
    payRow.appendChild(prevBtn);
    payRow.appendChild(buyBtn);

    // Descargar (deshabilitado hasta pago) — va debajo de Anterior/Comprar
