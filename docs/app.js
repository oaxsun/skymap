...
<svg xmlns="http://www.w3.org/2000/svg" width="${gap}" height="${gap}">
  ...
  function applyStepFromUrl(){
    const qp = new URLSearchParams(window.location.search);
    const step = (qp.get("step") || "").toLowerCase().trim();

    // Acepta step=1|2|3 o nombres
    if (step === "3" || step === "export") state.step = 2;
    else if (step === "2" || step === "content") state.step = 1;
    else if (step === "1" || step === "design") state.step = 0;
  }
...
function initPaidStateFromUrl(){
    ...
    // UX: si vienes de Stripe, llévate directo a Exportar (Paso 3)
    state.step = 2;
  ...
  async function downloadPDFfromCanvas(posterCanvas, Wpx, Hpx, dpi, filename){
    ...
    // Descarga directa (sin popups / sin pestañas nuevas)
    const a = document.createElement("a");
    a.href = url;
    a.download = filename || "skymap.pdf";
    document.body.appendChild(a);
    a.click();
    a.remove();
...
const yDT       = Math.round(relTop(pDTEl)    * sy);const title = String(state.text.title || "").slice(0, TITLE_MAX);
    ...
    // Tamaño (selector simple como la Fuente)
