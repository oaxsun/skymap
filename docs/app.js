(() => {
  ...
  const EXPORT_SIZES = [
    { key: "S",  title: "S",  sub: "A4 · 21 × 29.7 cm",   type: "cm", w: 21,   h: 29.7 },
    { key: "M",  title: "M",  sub: "A2 · 42 × 59.4 cm",   type: "cm", w: 42,   h: 59.4 },
    { key: "L",  title: "L",  sub: "A1 · 59.4 × 84.1 cm", type: "cm", w: 59.4, h: 84.1 },
    { key: "XL", title: "XL", sub: "A0 · 84.1 × 118.9 cm",type: "cm", w: 84.1, h: 118.9 }
  ];
...
<svg xmlns="http://www.w3.org/2000/svg" width="${gap}" height="${gap}">
  ...
  // ===========================
  // ==========================================================
  // ✅ Export PDF via backend (mantiene preview en el front)
  // - El render del poster (canvas) sigue en el navegador.
  // - El PDF final se arma en backend.
  // ==========================================================

  function getApiBase(){
    // Puedes sobreescribirlo desde generator.html:
  ...
  function restoreCheckoutDraftIfAny(){
    try{
      const raw = localStorage.getItem(CHECKOUT_DRAFT_KEY);
      if (!raw) return;
      const data = JSON.parse(raw);
      if (!data?.state) return;
      if (Date.now() - Number(data.ts || 0) > 6 * 60 * 60 * 1000) return; // 6h

      Object.assign(state.map, data.state.map || {});
      Object.assign(state.text, data.state.text || {});
      Object.assign(state.visible, data.state.visible || {});
      if (data.state.export){
        state.export.sizeKey = data.state.export.sizeKey || state.export.sizeKey;
        state.export.format = data.state.export.format || state.export.format;
        state.export.dpi = data.state.export.dpi || state.export.dpi;
      }
    }catch(_e){}
  }
  ...
  async function ensurePaidFromUrl(){
    ...
    if (!sessionId){
      alert("Para descargar, primero completa tu pago.");
      return false;
    }

    try{
      const api = getApiBase();
      const r = await fetch(`${api}/api/payment-status?session_id=${encodeURIComponent(sessionId)}`);
      const data = await r.json().catch(()=> ({}));

      if (!r.ok) throw new Error(data?.error || "No se pudo verificar el pago");

      if (!data?.paid){
        alert("Pago aún no confirmado. Si acabas de pagar, espera unos segundos y recarga.");
        return false;
      }

      state.export.paid = true;
      state.export.sessionId = sessionId;
      return true;
    }catch(e){
      console.error(e);
      alert("No se pudo verificar el pago. Intenta recargar la página.");
      return false;
    }
  }

  function initPaidStateFromUrl(){
    const qp = new URLSearchParams(window.location.search);
    const sessionId = qp.get("session_id");
    if (!sessionId) return;
  ...
  async function downloadPDFfromCanvas(posterCanvas, Wpx, Hpx, dpi, filename){
    const blob = await new Promise((resolve, reject) => {
      posterCanvas.toBlob((b) => b ? resolve(b) : reject(new Error("No pude generar blob JPEG")), "image/jpeg", 0.98);
    });

    const form = new FormData();
    form.append("file", blob, "poster.jpg");
    form.append("wpx", String(Wpx));
    form.append("hpx", String(Hpx));
    form.append("dpi", String(Number(dpi) || 300));
    form.append("filename", filename || "skymap.pdf");
    ...
    const api = getApiBase();
    const r = await fetch(`${api}/api/pdf`, { method: "POST", body: form });

    if (!r.ok){
      const txt = await r.text().catch(()=> "");
      throw new Error(`Error PDF (${r.status}): ${txt || "No se pudo generar el PDF"}`);
    }
...
const yDT       = Math.round(relTop(pDTEl)    * sy);const title = String(state.text.title || "").slice(0, TITLE_MAX);
    ...
    const s = document.createElement("div");
    s.className = "sub";
    s.textContent = "Selecciona el tamaño de tu poster, paga y descarga. ¡Así de fácil!";
    ...
    sizeWrap.style.gap = "10px";

    const sizeOptions = [
      { key: "S",  label: "S",  sub: "A4 · 21 × 29.7 cm" },
      { key: "M",  label: "M",  sub: "A2 · 42 × 59.4 cm" },
      { key: "L",  label: "L",  sub: "A1 · 59.4 × 84.1 cm" },
      { key: "XL", label: "XL", sub: "A0 · 84.1 × 118.9 cm" },
    ];
    ...
    sizeOptions.forEach(opt => {
      ...
      row.style.border = "1px solid rgba(233,238,252,.10)";
      ...
      radio.type = "radio";
      radio.name = "exportSize";
      radio.value = opt.key;
      radio.checked = state.export.sizeKey === opt.key;
      radio.onchange = () => { state.export.sizeKey = opt.key; };

      const txt = document.createElement("div");
      txt.innerHTML = `<div style="font-weight:800">${opt.label}</div><div style="font-size:12px; color: rgba(233,238,252,.70)">${opt.sub}</div>`;
    ...
    formatWrap.style.gridTemplateColumns = "1fr 1fr";
    formatWrap.style.gap = "10px";
    ...
    formats.forEach(f => {
      ...
      btn.style.border = "1px solid rgba(233,238,252,.10)";
      btn.style.background = (state.export.format === f.key) ? "rgba(124,255,250,.12)" : "rgba(255,255,255,.02)";
      btn.style.color = "rgba(233,238,252,.92)";
      btn.textContent = f.label;
      btn.onclick = () => { state.export.format = f.key; renderAll(); };
      formatWrap.appendChild(btn);
    });

    formatRow.appendChild(formatWrap);

    // Descargar (deshabilitado hasta pago)
    const downloadRow = document.createElement("div");
    downloadRow.className = "formRow";
    ...
    downloadBtn.onclick = async () => {
      if (!state.export.paid){
        ...
        if (!ok) return;
        renderAll(); // re-render para habilitar estilos
      }
    ...
    payWrap.className = "formRow";
    payWrap.style.display = "grid";
    ...
    payWrap.appendChild(buyBtn);
    payWrap.appendChild(prevBtn);
