...
const yDT       = Math.round(relTop(pDTEl)    * sy);const title = String(state.text.title || "").slice(0, TITLE_MAX);
    ...
    EXPORT_SIZES.forEach(sz => {
      const opt = document.createElement("option");
      opt.value = sz.key;
      // Ej: "S — 21 cm x 29.7 cm (A4)"
      opt.textContent = `${sz.key} — ${sz.sub}`;
      sizeSel.appendChild(opt);
    });

    sizeSel.value = state.export.sizeKey || EXPORT_SIZES[0].key;
    sizeSel.onchange = () => {
      state.export.sizeKey = sizeSel.value;
      // si en algún punto mostramos precio dinámico, lo podemos recalcular aquí
    };
